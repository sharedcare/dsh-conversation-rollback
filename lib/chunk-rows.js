/**
 * VENDORED PORT of dsh-session's internal `lib/types/chunk-rows.js`
 * (dsh 0.1.5-rc.1). That module is still shipped inside the package but is no
 * longer reachable through any public surface: it is absent from the root
 * entry, from the `./types` subpath, and from the package `exports` map — so
 * the rollback host carries its own copy to stay independent of the package's
 * private layout. Only the two brand imports were inlined: `brandString` is
 * runtime identity, and `SessionSeq` is a non-negative-safe-integer check.
 * The physical row vocabulary is a released-format concern (v3 still decodes
 * these rows through the frozen v0 codec), so this codec stays correct as
 * long as the released formats keep the same row tags. Keep this file in sync
 * with the upstream module if the physical row vocabulary ever changes.
 *
 ...
 *
 * @module conversation-rollback/chunk-rows
 */
/** Brand a value as a Session sequence. Runtime is a validation only. */
